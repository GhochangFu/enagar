...
| **13** | **Sprint 6.6 — catalogue alignment** | Master · Phase 6 | **Citizen runtime ← DB:** switch **`GET /services/tenants/:code`** + PWA/mobile apply to Postgres **`TenantService`** + published **`service_form_versions`** + effective fee/docs from 6.3; retire seed / **`service-schemas.ts`** as primary SoT — [§ Phase 6](#phase-6--admin-portals-state--tenant) |
| **14** | **Sprint 6.7 — designer polish**     | Master · Phase 6 | **Admin UX:** drag-drop form palette + React Flow / X6 workflow canvas; persists the same JSON / tables as Sprint 6.2 — [§ Phase 6](#phase-6--admin-portals-state--tenant)                                                                                                                              |
...
**Queued (#13–#14):** **#13 Sprint 6.6** (catalogue alignment) → **#14 Sprint 6.7** (designer polish). Replan with sponsor only via explicit exception.
...
**Sprint 6.6 (#13) — catalogue alignment (planned):** Wire citizen PWA + mobile to effective tenant catalogue and published form/workflow/config from admin portals; satisfies Phase 6 exit criterion “citizen sees published service within minutes.”

**Sprint 6.7 (#14) — designer polish (planned):** Visual form builder + workflow graph editor; no change to publish contracts from Sprint 6.2.
...
- **Sprint 6.5** (queue **#12**): State Super-Admin Portal + tenant onboarding wizard + impersonation + cross-tenant analytics.
- **Sprint 6.6** (queue **#13**): **Catalogue alignment** — citizen PWA/mobile consume DB-backed catalogue + published forms/config (defers from 6.1–6.3).
- **Sprint 6.7** (queue **#14**): **Designer polish** — drag-drop form palette + visual workflow canvas (defers from 6.2).
...
**Current state**: **Phase 2 complete.** **Master Phase 4:** **Sprints 4.1–4.2** closed **2026-05-11**; **Sprint 4.3 core** closed **2026-05-13**; **Master Phase 4 backlog slice (#3)** **closed engineering 2026-05-14** ([exit record](./docs/runbooks/phase4-backlog-slice-exit.md)). Phase 3 payment/finance slices through **3.3A** closed on stub rail; **Sprint 3.1B** **deferred**. **Citizen Unified Hub programme** **H1–H4** closed; **Hub H5.1** repo engineering slice merged; **Hub H6.1** **closed in-repo (engineering, 2026-05-14)** per [exit checklist](./docs/runbooks/hub-h6-exit-checklist.md) ([runbook](./docs/runbooks/citizen-unified-hub.md)); sponsor PO line optional. **Master Sprint 5.1 (#4)** and **5.2 (#5)** (5.2a+5.2b), **Sprint 5.3 (#6)**, **Sprint 5.4 (#7)** **closed in-repo (engineering)** — [`master-sprint-54-exit.md`](./docs/runbooks/master-sprint-54-exit.md) closes push/deep-link/PWA polish. **Sprints 6.1–6.4 (#8–#11)** **closed in-repo (engineering)** — [`master-sprint-61-exit.md`](./docs/runbooks/master-sprint-61-exit.md), [`master-sprint-62-exit.md`](./docs/runbooks/master-sprint-62-exit.md), [`master-sprint-63-exit.md`](./docs/runbooks/master-sprint-63-exit.md), [`master-sprint-64-exit.md`](./docs/runbooks/master-sprint-64-exit.md). **Locked queue next:** **#12 Sprint 6.5**; then **#13** (6.6 catalogue alignment), **#14** (6.7 designer polish). See [locked queue](#locked-sprint-queue-priority-order-114).
...
**Next action:** Continue Phase **6** with **Sprint 6.5** (queue **#12**) — State Super-Admin Portal, tenant onboarding wizard, impersonation, and cross-tenant analytics. Then **#13** (6.6 catalogue alignment), **#14** (6.7 designer polish). Parallel **Hub H5.1** staging / pilot readiness as capacity allows. **Sprint 3.1B** (payment gateway) stays **deferred** until sponsor unpause. _(Hub **H5.1** and Master **Phase 5** sprints are distinct labels — do not confuse.)_
