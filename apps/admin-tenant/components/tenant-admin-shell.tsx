...
import { Button, Icon, OperatorAppFooter, OperatorSidebarBrand, ToastProvider } from '@enagar/ui';
...
import { TenantAdminTopBar } from './tenant-admin-topbar';
...
function breadcrumbForPath(pathname: string): string {
  ...
  if (pathname.startsWith('/dashboard/masters')) return 'Masters';
      ...
      <div className="flex min-h-screen bg-canvas">
        ...
        >
          <div className="border-b border-sidebar-border px-3 py-3">
            ...
            {!collapsed ? (
              <p className="mt-2 truncate rounded-xl bg-sidebar-muted px-2.5 py-1.5 font-mono text-xs font-medium text-sage">
            ...
            {navItems.map((item) => {
              ...
              if (item.disabled) {
                return (
                  <p
                    key={item.id}
                    className={`${baseClass} cursor-not-allowed text-ink-onDarkMuted/60`}
                    title="Admin only"
                  >
                    <Icon name={item.icon} size={18} className="opacity-50" />
                    {!collapsed ? (
                      <span>
                        {item.label}
          ...
          {!collapsed ? (
            <div className="border-t border-sidebar-border px-3 py-3 text-center text-[10px] text-ink-onDarkMuted">
              © WB Govt · eNagarSeba
            </div>
          ) : null}
        </aside>
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          ...
          <TenantAdminTopBar
            breadcrumb={
              <p className="truncate text-sm text-ink-secondary">
                <span className="font-semibold text-ink-primary">{tenantCode ?? 'Tenant'}</span>
          ...
          <main className="flex flex-1 flex-col overflow-x-auto">
            ...
            <OperatorAppFooter operatorHelpHref="/help/operator-help-admin-tenant.html" />
          </main>
        </div>
      </div>
    </ToastProvider>
