...
- **Services & applications (Phase 2 + Sprint 5.3 spine + 5.4 PWA + 6.6 + upload programme 6.26):** tenant catalogue, **`createRenderPlan`** (**`platform: 'web'`**) via **`@enagar/forms/web`** (`DynamicFormFields` on **`@enagar/ui`**), draft → **`@enagar/forms/upload`** (`putFileToUploadUrl` → `confirm-upload` → poll `waitForDocumentScan` or dev simulation) → submit, **My Applications** with detail + comments and **Download** when `scan_status=clean`. Runtime form schemas come from API-published `service_form_versions`; **`lib/service-schemas.ts`** keeps dev smoke defaults only. **Installable PWA:** `app/manifest.ts`, `public/sw.js`, `app/icon.tsx`; deep links **`?grievance=`** / **`?application=`** (session required); optional **`NEXT_PUBLIC_VAPID_PUBLIC_KEY`** for web push → **`POST /citizen/notifications/push-token`**.
...
### Manual smoke — Object storage upload programme (6.26 / 6.28)

**Prerequisites:** API `:3001`, `OBJECT_STORAGE_DISABLED=false`, MinIO + `pnpm infra:minio-cors`, dev OTP `12345`.

1. Hub → pick **KMC** → **Services** → **Birth Certificate** → attach PDF on `hospital_discharge` → submit (wait for scan clean).
2. **My Applications** → open docket → **Download** on clean document.
3. **Grievances** → file with photo (real PUT when storage on) → confirm on Tenant Desk **:3002** evidence grid (not SVG stub).

**Automated API replay:** `node scripts/smoke-sprint-626.mjs` from repo root (same upload contract). Programme exit: `node scripts/smoke-sprint-630-programme.mjs`.
...
| **Sprint 6.11**    | Closed Phase 6 P4: PDF reports, rich KB/RAG trigger, branding assets, and bookable assets/calendar MVP — see [`master-sprint-611-exit.md`](../../docs/runbooks/master-sprint-611-exit.md)                                               |
