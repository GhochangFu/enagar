...
- **Services & applications (Phase 2 + Sprint 5.3 spine + 5.4 PWA):** tenant catalogue, **`createRenderPlan`** (**`platform: 'web'`**) via **`@enagar/forms/web`** (`DynamicFormFields` on **`@enagar/ui`**), draft → document scan simulation → submit, **My Applications** with detail + comments (writes include scope header when in workspace). Fixture map + smoke defaults: **`lib/service-schemas.ts`** (mirrors **`@enagar/mobile`**). **Installable PWA:** `app/manifest.ts`, `public/sw.js`, `app/icon.tsx`; deep links **`?grievance=`** / **`?application=`** (session required); optional **`NEXT_PUBLIC_VAPID_PUBLIC_KEY`** for web push → **`POST /citizen/notifications/push-token`**.
