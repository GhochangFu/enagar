...
| **Grievance Category** | A top-level grouping. **10 categories**: Sanitation & Waste, Water Supply, Roads & Infrastructure, Street Lighting, Public Health & Safety, Property & Taxation, Encroachment & Illegal, Environment, Service Delivery, Emergency. | See `docs/service-catalogue.md` §3.      |
| **Sub-type**           | A finer-grained classification within a Category. Drives routing to the right Field Staff. Examples: _Garbage not collected_, _Pothole_, _Streetlight not working_.                                                                | 30-50 sub-types total (Phase 4).         |
