...
| **Department**                 | An organisational unit within a Tenant (ULB), e.g. _Advertising & Hoarding_, _Health_. Owns tenant service categories and department-scoped designations. Configured in Tenant Admin Masters.                                                        | `tenant_departments`                   | Not a Keycloak role. See [ADR-0011](./ADRs/ADR-0011-org-designations-dept-catalogue.md).   |
| **Designation**                | A post title or duty assignment used in workflows (e.g. _Hoarding Inspector_, _Executive Officer_). Codes are **defined per ULB** (no State master list). Scope is `department` or `municipality`.                                                   | `tenant_designations`                  | Workflow authority; distinct from coarse JWT `role`.                                       |
| **Board of Councillors (BOC)** | Municipality-level oversight body. When service policy requires it, a **BOC resolution** must be recorded in-system (metadata + optional document) before final approval. Stage remains in the workflow graph; **guards** skip it when not required. | `board_of_councillors` (example code)  | See `docs/workflow-designations.md` §5.                                                    |
| **Chairperson**                | Municipal signatory at top of EO → CIC → VC → Chairperson ladder (high-value works). May **reject** applications (`can_reject_municipal`). Distinct from BOC.                                                                                        | `chairperson`                          | [ADR-0012](./ADRs/ADR-0012-post-approval-execution.md).                                    |
| **Work Order**                 | Execution record after approval and (if required) payment — one linked `work_orders` row per application (v1); assignment to internal staff or registered vendor; citizen feedback after completion.                                                 | `work_orders`                          | [ADR-0012](./ADRs/ADR-0012-post-approval-execution.md) Option A.                           |
...
| **Service Category**        | A **global** grouping for citizen navigation (14 codes: `cert`, `adv`, …). See `docs/service-catalogue.md` §2. Each tenant service also belongs to a **Tenant Service Category** under a Department (ADR-0011).                        | Citizen UI uses global category + optional department filter.                                          |
| **Tenant Service Category** | A department-owned catalogue grouping (e.g. _Hoarding permissions_ under Advertising & Hoarding). `tenant_services.category_id` references this row.                                                                                   | `tenant_service_categories`                                                                            |
...
| **Stage**       | A node in a Workflow. Holds: name, owner (designation or legacy role), allowed transitions, side-effects, SLA timer, optional guards. Examples: _submitted_, _site-inspection_, _boc-resolution_, _certificate-issued_. | Per-service definition; shared primitives in `@enagar/workflow`.                                                                      |
| **Transition**  | A guarded move from one Stage to another. Carries a `verb`, optional **guard** (e.g. BOC required), comment, and audit row.                                                                                             | Actor is a **designation** (preferred) or legacy **role**; see [ADR-0011](./ADRs/ADR-0011-org-designations-dept-catalogue.md).        |
...
| **Pending At**  | The designation (or legacy role) responsible for the next action. Citizen UI example: _"Pending at: Hoarding Inspector — Advertising & Hoarding"_.                                                                      | `pending_designation` (preferred) or `pending_role` (legacy).                                                                         |
...
| **forward**               | Operator action: move to the **next** internal workflow stage (maker/checker/municipal ladder).          | Operator (designation)  | Prefer over generic _approve_ in designation workflows.                                         |
| **return**                | Operator action: move to the **previous internal** stage (one step back on the graph). Not the citizen.  | Operator (designation)  | Requires explicit return edge. See [workflow-designations.md](./workflow-designations.md) §4.5. |
...
| **approve**               | Legacy/generic forward synonym in role-based workflows; new designation templates use **forward**.       | Operator                | Stage transition + audit.                                                                       |
| **reject**                | Terminal failure with reason. In designation workflows: **department head** or **Chairperson** only.     | Dept head / Chairperson | Final stage _rejected_.                                                                         |
