...
| Path                                                                                               | Purpose                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| -------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`infrastructure/keycloak/realm-export.json`](../../infrastructure/keycloak/realm-export.json)     | Partial realm JSON — clients (including **`admin-tenant` password grant enabled for dev smoke only**), realm roles, declarative **`userProfile.unmanagedAttributePolicy: ADMIN_EDIT`** (so scripted `tenant_id` / `tenant_code` persist), **`tenant-claims`** scope with mappers (`audience-enagar-api`, `tenant_*`, `role`, **`sub-username`** maps username → access-token `sub`). Extend when roles/clients change; import via Docker `start-dev --import-realm` or Admin UI / `kcadm.sh`. |
...
| `admin-tenant` | Tenant Admin portal (`http://localhost:3002/*` in dev export). **Direct access grants** are **on** in this repo’s export so local **password-grant** smoke scripts work; **turn off** for hardened / production-style portals and use **Authorization Code + PKCE** only. |
| `admin-state`  | State Admin portal                                                                                                                                                                                                                                                        |
...
**User profile:** the export sets `userProfile.unmanagedAttributePolicy = ADMIN_EDIT` so the seed script’s `tenant_id` / `tenant_code` attributes are not silently discarded. If your Keycloak build ignores this field during import, set the same policy under **Realm settings → User profile** (allow administrators to edit unmanaged attributes), then re-run **`pnpm infra:seed-keycloak-users`**.
