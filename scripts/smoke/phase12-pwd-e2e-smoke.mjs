/**
 * Phase 12 API smoke — payment-received → work order → desk execution → citizen feedback.
 *
 * Prereq: API :3001, Keycloak :8080, DB seeded, APPLICATION_STORE_PROVIDER=postgres
 *
 * Usage: pnpm smoke:phase12-pwd
 */
import { readFileSync, existsSync } from 'node:fs';
import { dirname, resolve } from 'node:path';
import { fileURLToPath } from 'node:url';

import { phase12PwdSmokeWorkflow } from './lib/phase12-pwd-workflow.mjs';
...
const TENANT = 'KMC';
const SERVICE_CODE = process.env.PHASE12_SERVICE_CODE ?? 'trade-licence';
...
function log(step, detail) {
  console.log(`[phase12-pwd] ${step}${detail ? `: ${detail}` : ''}`);
}

function fail(message) {
  console.error(`[phase12-pwd] FAIL: ${message}`);
  process.exit(1);
}
...
async function fetchDesk(token, docketNo) {
  const { res, json, text } = await api(
    'GET',
    `/admin/tenant/desk/applications/${encodeURIComponent(docketNo)}`,
    token,
  );
  assertOk('desk detail', res.status, text);
  return json;
}

async function main() {
  log('start', 'PWD post-approval execution smoke');
  ...
  const workflow = phase12PwdSmokeWorkflow(serviceCode);
  ...
  await api(
    ...
    { name: 'Phase12 PWD Smoke', mobile: citizenMobile },
  ...
  const { res: payInitRes, json: appPayment } = await api(
    ...
    {
      'x-enagar-tenant-code': TENANT,
      'idempotency-key': `phase12-app-fee-${draft.id}`,
    },
  );
  ...
  let detail = await fetchDesk(clerkTok, docketNo);
  if (detail.application.current_stage !== 'payment-pending') {
    fail(`expected payment-pending, got ${detail.application.current_stage}`);
  }

  const activePaymentId = detail.application.active_payment_id;
  ...
  await api(
    ...
    '/payments/stub/complete',
    citizenTok.access_token,
    { payment_id: activePaymentId, gateway_order_id: `stub_order_${activePaymentId}` },
    { 'x-enagar-tenant-code': TENANT },
  ).then(({ res, text }) => assertOk('stub approval fee', res.status, text));

  detail = await fetchDesk(clerkTok, docketNo);
  if (detail.application.current_stage !== 'work-order-issued') {
    fail(`expected work-order-issued after pay, got ${detail.application.current_stage}`);
  }
  if (!detail.work_order?.work_order_no) {
    fail('work_order missing after create_work_order effect');
  }
  log('work-order', detail.work_order.work_order_no);

  const vendor = detail.vendors?.find((row) => row.code === 'pwd-contractor-alpha');
  if (!vendor) fail('seed vendor pwd-contractor-alpha missing');
  await api(
    'PATCH',
    `/admin/tenant/desk/applications/${applicationId}/work-order`,
    clerkTok,
    { vendor_id: vendor.id },
  ).then(({ res, text }) => assertOk('assign vendor', res.status, text));

  await deskTransition(clerkTok, applicationId, 'forward');
  detail = await fetchDesk(clerkTok, docketNo);
  if (detail.application.current_stage !== 'work-in-progress') {
    fail(`expected work-in-progress, got ${detail.application.current_stage}`);
  }

  await deskTransition(clerkTok, applicationId, 'forward');
  await deskTransition(clerkTok, applicationId, 'forward');
  detail = await fetchDesk(clerkTok, docketNo);
  if (detail.application.current_stage !== 'citizen-feedback') {
    fail(`expected citizen-feedback, got ${detail.application.current_stage}`);
  }

  const { res: feedbackRes, json: closedApp } = await api(
    'POST',
    `/applications/${applicationId}/feedback`,
    citizenTok.access_token,
    { rating: 5, comment: 'Work completed satisfactorily.' },
    { 'x-enagar-tenant-code': TENANT },
  );
  assertOk('citizen feedback', feedbackRes.status, JSON.stringify(closedApp));
  if (closedApp.current_stage !== 'closed') {
    fail(`expected closed after feedback, got ${closedApp.current_stage}`);
  }
  if (!closedApp.citizen_feedback?.rating) {
    fail('citizen_feedback missing on application');
  }

  log('done', docketNo);
  console.log('[phase12-pwd] PASS');
}
