...
**Citizen PWA (`apps/citizen-pwa`) — Sprint 4.2 + 4.3 + Phase 4 backlog + Sprints 5.3–5.4:** authenticated **Grievances** tab (same API contract as above). **Apply** flows use **`createRenderPlan`** (**`platform: 'web'`**) via **`@enagar/forms/web`** (`DynamicFormFields`) on **`@enagar/ui`** primitives; fixture maps + smoke defaults live in **`lib/service-schemas.ts`** (aligned with **`@enagar/mobile`**). List view surfaces unread **`sla_breach`** inbox notices when `GET /citizen/notifications` returns unread rows. **PWA:** **`app/manifest.ts`**, **`public/sw.js`**, dynamic **`app/icon.tsx`**; query **`?grievance=`** / **`?application=`** deep links (session required); optional web push with **`NEXT_PUBLIC_VAPID_PUBLIC_KEY`**. Exit for the **5.3** spine: [`master-sprint-53-exit.md`](docs/runbooks/master-sprint-53-exit.md); **5.4:** [`master-sprint-54-exit.md`](docs/runbooks/master-sprint-54-exit.md).

**Citizen mobile (`apps/mobile`) — Master Sprint 5.1–5.2 + 5.4:** Expo **SDK 52** — **Splash** → **`GET /api/tenants`** → **OTP** → **Home** → **grievances** (**`/grievances`** + Bearer + **`x-enagar-tenant-code`**) · **services / applications / payments** (**`/services/tenants/:code`**, **`/applications/**`, **`/payments/**`**, **`/documents/**`** simulation) with **`createRenderPlan`** (**`platform: 'native'`**) from **`@enagar/forms/fixtures`**. Offline grievance composer drafts (**`@enagar/forms`** envelope). **Push:** Expo token → **`POST /citizen/notifications/push-token`**. **Deep links:** **`enagarseba://`** + React Navigation **`linking`**. Exit: [`master-sprint-51-exit.md`](docs/runbooks/master-sprint-51-exit.md), [`master-sprint-52-exit.md`](docs/runbooks/master-sprint-52-exit.md), [`master-sprint-52b-exit.md`](docs/runbooks/master-sprint-52b-exit.md), [`master-sprint-54-exit.md`](docs/runbooks/master-sprint-54-exit.md).
